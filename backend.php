	...
	if( !class_exists( 'rDepoimentos_Backend' ) ){
		class rDepoimentos_Backend {
			...
			public static function _cpt(){
				...
				$td = 'r-'. self::$cpt;
				$single = __( 'Depoimento', $td );
				$plural = __( 'Depoimentos', $td );
				
				$labels = apply_filters( 'r/depoimentos/cpt/labels', array(
					'name'                  => _x( 'Depoimento', 'Post Type General Name', $td ),
					'singular_name'         => _x( 'Depoimento', 'Post Type Singular Name', $td ),
					'menu_name'             => $plural,
				...
				$supports = apply_filters( 'r/depoimentos/cpt/supports', array( 'title', 'editor', 'thumbnail', 'revisions', 'page-attributes' ) );
				
				$args = apply_filters( 'r/depoimentos/cpt/args', array(
					'label'                 => $single,
					...
					'show_in_menu'          => true,
					'menu_position'         => 25,
					'menu_icon'				=> apply_filters( 'r/depoimentos/cpt/icon', 'dashicons-testimonial' ),
			...
			public static function _post_type_link( $url, $post, $leavename ){
				$base = apply_filters( 'r/depoimentos/base_url', home_url( self::$cpt .'/' ), $url, $post );
				if( $post->post_type == self::$cpt ){
					$url = apply_filters( 'r/depoimentos/item_url', $base .'#'. $post->post_name, $base, $post->ID );
				}
				return $url;
			}
			...
			public static function _sc( $args = array(), $content ){
				...
				// args[ template ]
				$theme_templates = apply_filters( 'r/depoimentos/sc/templates', array(
					'single-depoimentos.php',
					'partials/depoimentos/_item.php'
				) );
				...
				$template_file = R_DEPOE_PATH .'views/_item.php';
				...
				if( isset( $args[ 'class' ] ) && !empty( $args[ 'class' ] ) ){
					#var_dump( [ 'args.class' => $args[ 'class' ] ] );
				...
				*/
				...
				if( $loop->have_posts() ){
					while( $loop->have_posts() ){
						...
						$template_file = apply_filters( 'r/depoimentos/sc/template_file', $template_file, get_the_ID() );
						...
						$item = apply_filters( 'r/depoimentos/sc/item_html', $item, get_the_ID() );
				...
				// items
				$items_html = implode( "\n", $items );
				$items_html = apply_filters( 'r/depoimentos/sc/items_html', $items_html, $loop );

				
				// items wrap
				$atts[ 'class' ] = implode( " ", (array)$atts[ 'class' ] );
				$items_atts = apply_filters( 'sc/depoimentos/items_attrs', $atts, $loop );
				$items_wrap = rHtml::tag( $tag, $items_atts, $items_html );
				

				// template file data
				$template_file = str_replace( '\\', '/', $template_file );
				if( strpos( $template_file, 'depoimentos/' ) !== false ){
					$template_arr = explode( 'depoimentos/', $template_file );
				...
				// output
				$output_atts = array( 'class' => 'depoimentos-wrap', 'data-template' => $template_name );
				$output_atts = apply_filters( 'r/depoimentos/sc/output_attrs', $output_atts );
				
				$output = rHtml::tag( 'div', $output_atts, $items_wrap );
				$output = apply_filters( 'r/depoimentos/sc/output', $output, $loop );
